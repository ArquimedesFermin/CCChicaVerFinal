...
namespace CChica.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        public ActionResult ChangePasswordAdmin()
        {
            ...
            {
                ViewBag.consultaUser = db.Users.Select(r => new { r.Id, r.UserName }).ToList();
            ...
            {
                return RedirectToAction("NotAuto","Account");
            }
        }
        ...
        public async Task<ActionResult> ChangePasswordAdmin(ChangePasswordViewModel model)
        {
            ...
            // var userPassword = db.Users.Where(a => a.Id == model.id).Select(r => r.PasswordHash).First();

            string code = await UserManager.GeneratePasswordResetTokenAsync(model.id);



           // model.OldPassword = userPassword;

            IdentityResult result = await UserManager.ResetPasswordAsync(model.id, code, model.NewPassword);
            if (result.Succeeded)
            {
                ...
                //    await SignInManager.SignInAsync(user, isPersistent: false, rememberBrowser: false);
                //}
                return RedirectToAction("Index","AddCajaChica", new { Message = ManageMessageId.ChangePasswordSuccess });
            }
            AddErrors(result);
            return View(model);
        }
